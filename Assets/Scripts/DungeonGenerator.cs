...
public class DungeonGenerator : MonoSingleton <DungeonGenerator> {
	...
	// List of rooms
    public List<Dungeon_Room> rooms;
...
//		GameObject.DestroyImmediate(floor);
    ...
    public void GenerateRooms(ref List<Dungeon_Room> _rooms, QuadTree _quadTree)
	{
		// Childless node
    ...
    public Dungeon_Room GenerateRoom(QuadTree _quadTree)
	{
		// Center of the room
		...
		// Return the room
        return new Dungeon_Room(randomAABB, _quadTree);
	}
    ...
    public Vector3 PlacePlayer()
    {
        int r = Random.Range(0, rooms.Count - 1);
        Dungeon_Room room = rooms[r];
        player.SetActive(true);
        return new Vector3(room.boundary.center.x, 2.0f, room.boundary.center.y);
    }
